...
export function useGetQuestionLean(
  ...
  const handleGetQuestionLearn: QueryFunction<ApiResponseType<LearnQuestionResponse>> = () =>
    responseWrapper<ApiResponseType<LearnQuestionResponse>>(LearnApi.getQuestionLearn, [
      options?.id,
    ]);
  ...
  const handleInvalidateQuestionLearn = () =>
    queryClient.invalidateQueries([ApiKey.LEARN, options?.id]);
