...
export function useGetQuestionPracticeCourseListen(
  ...
  const {
    ...
    [ApiKey.PRACTICE_COURSE_LISTENING, options.id],
    {
  ...
  const handleInvalidateQuestionPracticeCourseListen = () =>
    queryClient.invalidateQueries([ApiKey.PRACTICE_COURSE_LISTENING, options?.id]);
  ...
  return {
    ...
    isLoading: isLoading,
    isSuccess,
    handleInvalidateQuestionPracticeCourseListen,
  };
}
